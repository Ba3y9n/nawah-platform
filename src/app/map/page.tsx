...
export default function SmartMapPage() {
  const [sources, setSources] = useState<any[]>(VERIFIED_SOURCES);
      ...
      <div className="bg-white border border-emerald-200 p-6 rounded-3xl shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <select
            ...
            className="bg-slate-50 border border-emerald-200 rounded-2xl px-4 py-2.5 text-xs text-emerald-950 focus:outline-none focus:border-amber-400"
          >
          ...
          {selectedSource ? (
            <div className="bg-white border border-amber-400 rounded-3xl p-6 shadow-xl space-y-4 animate-in fade-in">
              ...
              <div className="space-y-2 text-xs bg-slate-50 p-4 rounded-2xl border border-emerald-100">
                ...
                <div className="flex justify-between">
                  <span className="text-emerald-700">تاريخ آخر تحقق:</span>
                  <span className="font-bold text-emerald-950">{selectedSource.last_verified_at}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-emerald-700">الدفعات المرتبطة:</span>
              ...
              <div className="space-y-2 max-h-[450px] overflow-y-auto pr-1">
                {filteredSources.map((s) => (
                  ...
                  >
                    ...
                    <p className="text-[10px] text-emerald-700 truncate">{s.data_source}</p>
                  </button>
                ))}
              </div>
            </div>
